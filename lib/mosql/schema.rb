module MoSQL
  ...
  class Schema
    ...
    def transform(ns, obj, schema=nil)
      ...
      mas_bson_size = 16*1024*1024 # keith: Hit limit https://github.com/stripe/mosql/issues/101
      obj = BSON.deserialize(BSON::BSON_CODER.serialize(obj, false, false, mas_bson_size))
    ...
    def make_children_chains(heads)
      ...
      children_chains = children_val_pairs.map.with_index do |pair, i|
        label = i.to_s
        chain_head = Node.new(label, pair[0])
        chain_tail = pair[1..-1].reduce(chain_head) do |parent, val|
          parent << Node.new(label, val)
        end
